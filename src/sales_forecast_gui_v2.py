...
ARIMA · Holt-Winters · Random Forest · Gradient Boosting · Ridge · Ensemble
Author: Abhishek 
"""
    ...
    def _run_ml_model(
        ...
        try:
            ...
            for _ in range(h):
                ...
                X_fut = X_fut_df.to_numpy(dtype=float)
                if X_fut.shape[1] != len(feat_names):
                    logger.warning("%s: feature shape mismatch at step %d, stopping.", model_name, _)
                    break
                if np.any(~np.isfinite(X_fut)):
...
class SalesAnalyzer:
    ...
    def decompose(
        ...
        if len(trend) != n:
            ...
            logger.warning("Trend length corrected from %d to %d", len(trend), n)
...
def main() -> None:
    ...
    app._log_msg("Sales Forecasting System ready.", "ok")
