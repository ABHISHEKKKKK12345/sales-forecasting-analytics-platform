...
║  ─────────────────────────────────────────────────────────────────────────  ║
║  Production-gradea forecasting engine · Statistical rigour · ML ensemble    ║
║                                                                             ║
║  Core Models:                                                               ║
...
║  • Correct import-guard: missing-dep dialog shown BEFORE mainloop           ║
║  • Dead code removed (ARIMA _undifference static / _diff_saves)             ║
║  • ML feature matrix built once per run (no redundant re-computation)       ║
║  • Ensemble weight NaN guard (min_delta = 1e-6)                             ║
║                                                                             ║
    ...
    def _run_ml_model(
        ...
        try:
            ...
            for _ in range(h):
                ...
                X_fut_df = X_fut_df.reindex(columns=feat_names, fill_value=0)
...
class PDFReportGenerator:
    ...
    def _kpi_table(self, st: Dict) -> Table:
        ...
        kpis = [
            ("Total Revenue",           f"${st['total_revenue']:,.0f}"),
            ("Avg Monthly Revenue",     f"${st['avg_monthly_revenue']:,.0f}"),
            ("CAGR",                    f"{st['cagr']:.1f}%"),
    ...
    def _build_story(self, analysis: SalesAnalysis, charts: Dict) -> List:
        ...
        story.append(Paragraph(
            ...
            "ARIMA: exact MLE via innovations form, ψ-weight CI propagation (dead _undifference removed). "
    ...
    def _pipeline(
        ...
        try:
            ...
            summary   = {
                ...
                "bug_fixes_applied": [
                    "BUG-1: import-guard: error dialog shown BEFORE mainloop",
                    "BUG-2: ARIMA dead code removed (_diff_saves / static _undifference)",
                    "BUG-3: ML feature matrix built once per run (no redundant build_features call)",
                    "BUG-4: ensemble weight NaN guard (min_delta=1e-6, uniform fallback)",
                ],
                "platform_info": {
                    "python":          sys.version,
...
def main() -> None:
    ...
    app._log_msg(f"Sales Forecasting System ready.", "ok")
    app._log_msg("⚙  Setup tab → configure → Run.", "accent")
    app._log_msg("── bug fixes ─────────────────────────────────", "sep")
    app._log_msg("[BUG-1] import-guard: error dialog fires BEFORE mainloop", "muted")
    app._log_msg("[BUG-2] ARIMA dead code removed (_diff_saves / _undifference)", "muted")
    app._log_msg("[BUG-3] ML feature matrix built once per run (no redundant call)", "muted")
    app._log_msg("[BUG-4] ensemble NaN guard: min_delta=1e-6 + uniform fallback", "muted")
    app._log_msg("────────────────────────────────────────────────────", "sep")
