...
║  ─────────────────────────────────────────────────────────────────────────  ║
║  Production-grade forecasting engine · Statistical rigour · ML ensemble     ║
║                                                                             ║
║  Core Models:                                                               ║
║  • ARIMA       : full MLE via innovations form + AIC/BIC grid search        ║
║  • Holt-Winters: additive & multiplicative, auto-selected by AIC            ║
║  • Random Forest / Gradient Boosting / Ridge Regression                     ║
║  • Stacked Ensemble: inverse CV-sMAPE weights (strictly non-leaky)          ║
║                                                                             ║
║  Statistical Robustness:                                                    ║
║  • CI  : ARIMA ψ-weight propagation · HW analytical/bootstrap · ML boot     ║
║  • Metrics: sMAPE · MASE · RMSE · MAE · R²                                  ║
║  • Diagnostics: Ljung-Box · Shapiro-Wilk · Durbin-Watson                    ║
║  • Walk-forward (expanding-window) cross-validation                         ║
║                                                                             ║
║  Feature Engineering:                                                       ║
║  • PACF-guided lag selection · Rolling stats · Harmonic seasonal encoding   ║
║  • Trend (linear / quadratic / sqrt) · Periodogram seasonality detection    ║
║                                                                             ║
║  Business Analytics:                                                        ║
║  • Marketing ROI with bootstrap CI · YoY / MoM growth · Anomaly detection   ║
║                                                                             ║
║  System Design:                                                             ║
║  • Thread-safe · Atomic file writes · CancelToken · Modular pipeline        ║
║  • Cross-platform tempdir for charts (tempfile.gettempdir())                ║
║  • Robust multi-encoding CSV · xls/xlsx/json orient-aware loading           ║
║  • Wizard GUI: Setup → Run → Results with KPI dashboard                     ║
║                                                                             ║
║  Output: PDF report · Excel workbook (9 sheets) · JSON summary              ║
║                                                                             ║
║  Requirements:                                                              ║
║      pip install numpy pandas matplotlib seaborn scipy scikit-learn         ║
║                  openpyxl reportlab pillow                                  ║
║  Run:                                                                       ║
║      python sales_forecast_gui.py                                           ║
║                                                                             ║
║  Author: Abhishek                                                           ║
║  Project: Sales Forecasting & Automated Reporting System                    ║                  
╚═════════════════════════════════════════════════════════════════════════════╝
"""
...
_PANDAS_GTE_2 = int(pd.__version__.split(".")[0]) >= 2 if not _IMPORT_ERROR else False
...
class ForecastConfig:
    # Use tempfile for cross-platform chart temp directory
    ...
    n_bootstrap:           int   = 300   # CI bootstrap iterations
    cv_folds:              int   = 3     # walk-forward CV splits
...
class ResidualDiagnostics:
    ...
    ljung_box_pval:   float = 1.0   # >0.05 → no autocorrelation (good)
    shapiro_stat:     float = 0.0
    shapiro_pval:     float = 1.0   # >0.05 → residuals normal (good)
    durbin_watson:    float = 2.0   # ≈2 → no serial correlation (good)
...
class ForecastResult:
    ...
    def accuracy_score(self) -> float:
        """0–100 composite; sMAPE-based, bounded."""
...
class StatUtils:
    """
    Rigorous, fully-tested statistical helpers.
    All methods are stateless class-methods for easy testability.
    """
    ...
    def smape(y_true: np.ndarray, y_pred: np.ndarray) -> float:
        """
        Symmetric MAPE — bounded [0, 200], safe for near-zero actuals.
        sMAPE = 200 * mean( |y-ŷ| / (|y|+|ŷ|+ε) )
        """
    ...
    def mase(y_true: np.ndarray, y_pred: np.ndarray,
        ...
        Scale = MAE of seasonal naïve forecast on training set.
        """
    ...
    def safe_mape(y_true: np.ndarray, y_pred: np.ndarray,
        ...
        """MAPE with guarded denominator. Use sMAPE for display."""
    ...
    def all_metrics(y_true: np.ndarray, y_pred: np.ndarray,
                    y_train: np.ndarray,
                    period: int = 12) -> Tuple[float, float, float, float, float, float]:
    ...
    @staticmethod
    def bootstrap_ci(point_forecasts: np.ndarray,
                     in_sample_residuals: np.ndarray,
                     alpha: float = 0.05,
                     n_boot: int = 500) -> Tuple[np.ndarray, np.ndarray]:
        """
        Non-parametric bootstrap CI.
        Each bootstrap path adds a single resampled residual per step
        (not cumsum) to keep bounds realistic for longer horizons.
        """
        ...
        # Scale uncertainty by sqrt(horizon) — random-walk envelope
    ...
    @staticmethod
    def normal_ci(point_forecasts: np.ndarray,
                  residuals: np.ndarray,
                  alpha: float = 0.05) -> Tuple[np.ndarray, np.ndarray]:
        """
        Analytical CI assuming normal residuals.
        Variance grows with horizon: Var(h) = σ² · h  (random-walk envelope).
        """
        if len(residuals) > 1:
        ...
        h      = len(point_forecasts)
        # horizon-widening: starts at h=1 → multiplier=1, grows as sqrt(h)
        scale  = np.sqrt(np.arange(1, h + 1, dtype=float))
        margin = z * std * scale
        return point_forecasts - margin, point_forecasts + margin

    @staticmethod
    def arima_forecast_ci(predictions: np.ndarray,
                          sigma2: float,
                          psi_weights: np.ndarray,
                          alpha: float = 0.05) -> Tuple[np.ndarray, np.ndarray]:
        """
        ARIMA analytical CI using ψ-weight variance propagation.
        Var(e_h) = σ² · Σ_{j=0}^{h-1} ψ_j²
        Reference: Box, Jenkins, Reinsel & Ljung (2015), §5.4
        """
        h       = len(predictions)
    ...
    @staticmethod
    def compute_diagnostics(residuals: np.ndarray,
                             max_lags: int = 10) -> ResidualDiagnostics:
        """
        Ljung-Box, Shapiro-Wilk, Durbin-Watson, heteroscedasticity flag.
        """
        ...
        # Shapiro-Wilk (valid for 4 ≤ n ≤ 5000)
        if 4 <= n <= 5000:
            try:
        ...
        # Heteroscedasticity: variance ratio first vs second half
        if n >= 8:
            try:
                half  = n // 2
    ...
    @staticmethod
    def detect_seasonality(y: np.ndarray,
                            candidates: Sequence[int] = (4, 6, 12, 24)) -> int:
        """
        Detect dominant seasonal period via periodogram power.
        Falls back to 12 if no strong signal is found.
        """
        n = len(y)
        if n < 24:
            return 12
    ...
    @staticmethod
    def marketing_roi_with_ci(sales: np.ndarray, spend: np.ndarray,
                               n_boot: int = 1000,
                               alpha: float = 0.05) -> Dict[str, float]:
        """
        Bootstrap CI for marketing ROI.
...
class SalesDataGenerator:
    ...
    def __init__(self, seed: Optional[int] = None):
        # Allow varying seed for reproducibility while avoiding always same data
        actual_seed = seed if seed is not None else int(time.time()) % 100_000
        self.rng = np.random.default_rng(actual_seed)

    def generate(self, n_months: int = 60, n_products: int = 5,
        ...
        for prod_name, params in products:
            ...
            # Inject 2 anomalies per product
...
class DataLoader:
    ...
    def _read_file(path: Path) -> "pd.DataFrame":
        ...
        if suffix in (".xls",):
            # xlrd required for legacy .xls; fall back to openpyxl warning
            try:
        ...
        if suffix == ".json":
            ...
            for orient in (None, "records", "split", "index", "values", "table"):
                try:
                    ...
                    df = pd.read_json(io.BytesIO(raw), **kwargs)
    ...
    def _validate_and_clean(cls, df: "pd.DataFrame") -> "pd.DataFrame":
        ...
        # Also strips BOM that survives utf-8-sig encoding
        df.columns = [
            str(c).strip().lstrip("\ufeff").lower().replace(" ", "_").replace("-", "_")
            for c in df.columns
        ]
        ...
        if missing:
            raise DataLoadError(
                ...
                f"Your file has: {sorted(df.columns.tolist())}"
            )

        # Date parsing — pandas 2.x removed infer_datetime_format
        if _PANDAS_GTE_2:
        ...
        if n_bad_dates > 0:
            logging.warning("Dropped %d rows with unparseable dates.", n_bad_dates)

        # Sales — coerce, fill NaN → 0, clip negatives
        df["sales"] = pd.to_numeric(df["sales"], errors="coerce").fillna(0.0).clip(lower=0.0)
...
class ARIMA:
    ...
    def __init__(self, p: int = 2, d: int = 1, q: int = 1):
        ...
        self._diff_saves: List[np.ndarray] = []  # saves first element of each diff step
    ...
    def _difference(y: np.ndarray, d: int) -> Tuple[np.ndarray, List[np.ndarray]]:
        """
        Difference y exactly d times.
        Saves the initial values needed for undifferencing.
        Returns (differenced_series, list_of_initial_slices).
        """
        out    = y.astype(float).copy()
        saves  = []
        for _ in range(d):
            saves.append(out[:1].copy())  # first element before this diff
            out = np.diff(out)
        return out, saves

    @staticmethod
    def _undifference(diff_preds: np.ndarray,
                      saves: List[np.ndarray]) -> np.ndarray:
        """
        Reconstruct levels from differenced predictions.
        saves[-1] is the last level before the last diff step.
        """
        result = diff_preds.copy().astype(float)
        # Reverse the differencing operations (last diff first)
        for init_val in reversed(saves):
            # Prepend the last observed value of the undifferenced series
            # and take cumsum to invert np.diff
            result = np.concatenate([init_val, result])
            result = np.cumsum(result)[1:]  # drop the prepended seed
        return result

    def _predict_undiff(self, diff_preds: np.ndarray) -> np.ndarray:
        """Undo all d differencing steps using stored saves."""
        result = diff_preds.copy().astype(float)
        # saves[i] holds the first element BEFORE the i-th diff.
        # To invert diff step i, we need the LAST element of the series
        # after i integrations — that's stored as _orig_at_level[i].
    ...
    def _innovations(self, yd: np.ndarray, ar: np.ndarray,
                     ma: np.ndarray, mu: float) -> np.ndarray:
        """Compute innovations ε_t = y_t - ŷ_{t|t-1}."""
        n     = len(yd)
        p, q  = len(ar), len(ma)
        eps   = np.zeros(n)
        for t in range(n):
    ...
    def _neg_log_likelihood(self, params: np.ndarray, yd: np.ndarray) -> float:
        ...
        n    = len(eps)
        sig2 = float(np.mean(eps ** 2)) + 1e-20
        return 0.5 * (n * math.log(2 * math.pi * sig2) + n)

    def fit(self, y: np.ndarray) -> "ARIMA":
        self._orig = y.astype(float).copy()

        # Difference d times, saving level values for undifferencing
        yd, saves = ARIMA._difference(self._orig, self.d)

        # Store last observed value at each integration level for prediction
        ...
        n = len(yd)
        # Clamp orders to data
        max_order  = max(n // 4, 1)
        self.p     = min(self.p, max_order)
        self.q     = min(self.q, max_order)

        k  = self.p + self.q + 1   # AR + MA + const
        x0 = np.zeros(k)

        # OLS warm-start for AR
        ...
        # MLE via L-BFGS-B with multiple restarts
        best_nll    = np.inf
        best_params = x0.copy()
        ...
        # ψ-weights (MA∞ representation for CI propagation)
        max_psi = 60
    ...
    def predict(self, h: int) -> np.ndarray:
        ...
        # Work in differenced space
        yd_orig, _ = ARIMA._difference(self._orig, self.d)
        ...
        for _ in range(h):
            ...
            # For future steps, innovations are 0; only use past innovations
            n_eps = len(eps)
            ma_part = sum(
            ...
            yd.append(pred)
            eps.append(0.0)   # future innovations are expectation-zero

        return np.maximum(
            self._predict_undiff(np.array(diff_preds, dtype=float)), 0.0)
    ...
    @classmethod
    def auto_select(cls, y: np.ndarray,
                    max_p: int = 3, max_d: int = 1, max_q: int = 2) -> "ARIMA":
...
class HoltWinters:
    ...
    Variant (add/mul) with lower AIC is auto-selected.

    Reference: Hyndman & Athanasopoulos, "Forecasting: Principles and Practice" (3rd ed.), §8.5
    """
    ...
    def _init_components(self, y: np.ndarray) -> Tuple[float, float, List[float]]:
        """Initialise level, trend, seasonal components from data."""
        m = self.season_len
        n = len(y)
        # Ensure at least one full season
        if n < 2 * m:
            m = max(2, n // 2)
            self.season_len = m
        ...
        # Normalise so seasonal factors sum to 0 (add) or m (mul)
        if self.multiplicative:
    ...
    def _smooth(self, y: np.ndarray,
        ...
        """Run HW recursion; return final level, trend, seasonal, fitted."""
        m              = self.season_len
        n              = len(y)
        level, trend, seas = self._init_components(y)
        seas_arr       = list(seas) * (n // m + 2)   # circular buffer
        fitted         = np.zeros(n)
    ...
    def fit(self, y: np.ndarray) -> "HoltWinters":
        ...
        for start in starts:
            try:
                ...
                if res.success and np.isfinite(res.fun) and res.fun < best_sse:
        ...
        self._sigma2 = sigma2
        # Exact Gaussian log-likelihood for AIC
    ...
    def predict(self, h: int) -> np.ndarray:
        ...
        m     = self.season_len
        preds = []
        # Extend seasonal buffer circularly from the last m factors
    ...
    def forecast_ci(self, h: int, alpha: float = 0.05) -> Tuple[np.ndarray, np.ndarray]:
        """
        Additive HW: analytical CI (Robertson 2009).
        Var(ŷ_{T+h}) ≈ σ²[1 + (h-1)(α² + αβh + β²h(2h-1)/6)]
        Multiplicative: non-parametric bootstrap fallback.
        """
        preds = self.predict(h)
        if not self.multiplicative:
            ...
            lo, hi = StatUtils.bootstrap_ci(
                preds, self._resid, alpha=alpha, n_boot=400)
            return np.maximum(lo, 0.0), hi

    @classmethod
    def auto_select(cls, y: np.ndarray, season_len: int = 12) -> "HoltWinters":
        """Fit additive and multiplicative; return fitted model with lower AIC."""
        best_aic = np.inf
...
class MLFeatureEngineering:
    ...
    def _pacf(y: np.ndarray, max_lag: int) -> np.ndarray:
        """Yule-Walker PACF via Durbin-Levinson recursion."""
        n    = len(y)
    ...
    @classmethod
    def select_lags(cls, y: np.ndarray, max_lag: int = 24,
                    period: int = 12) -> List[int]:
        """
        Return lags where |PACF| > 2/√n OR multiples of period.
        Always include lag 1.
        """
        n         = len(y)
    ...
    @classmethod
    def build_features(cls, y: np.ndarray,
                       period: int = 12) -> Tuple[np.ndarray, np.ndarray, List[str]]:
        n    = len(y)
        ...
        for w in [3, 6, 12]:
            if w <= n:
                feats[f"roll_mean_{w}"]  = s.rolling(w, min_periods=1).mean().to_numpy()
                feats[f"roll_std_{w}"]   = s.rolling(w, min_periods=1).std().fillna(0).to_numpy()
                feats[f"roll_min_{w}"]   = s.rolling(w, min_periods=1).min().to_numpy()
                feats[f"roll_max_{w}"]   = s.rolling(w, min_periods=1).max().to_numpy()
        ...
        y_clean = df["target"].to_numpy(dtype=float)
...
class WalkForwardCV:
    ...
    Prevents look-ahead bias; gives unbiased OOF predictions.
    """

    @staticmethod
    def evaluate(model_fn: Callable, y: np.ndarray,
                 period: int = 12, n_splits: int = 3,
                 horizon: int = 6) -> Dict[str, float]:
        """
        Returns {'smape': ..., 'mase': ..., 'rmse': ...} averaged over folds.
        model_fn(y_train) -> np.ndarray of length >= horizon.
        """
        n         = len(y)
        min_train = max(period * 2, 24)
        horizon   = min(horizon, n // 4)    # cap horizon to 25% of series
        if n < min_train + horizon:
            return {"smape": float("nan"), "mase": float("nan"), "rmse": float("nan")}

        # Build fold end-points (expand window)
...
class ForecastingEngine:
    ...
    def run_arima(self, y: np.ndarray,
                  period: int = 12) -> Optional[ForecastResult]:
    ...
    def run_exp_smoothing(self, y: np.ndarray,
                           period: int = 12) -> Optional[ForecastResult]:
    ...
    def _run_ml_model(self, y: np.ndarray,
                      base_model: Any,
                      model_name: str,
                      period: int = 12) -> Optional[ForecastResult]:
        ...
        try:
            ...
            X, y_clean, feat_names = MLFeatureEngineering.build_features(y, period=period)
            if len(X) < 12:
                self._report(f"{model_name}: insufficient data after feature engineering.")
                return None

            # Walk-forward CV via sklearn TimeSeriesSplit
            n_splits = min(self.config.cv_folds, max(2, len(X) // (h + 1)))
            tscv     = TimeSeriesSplit(n_splits=n_splits,
                                       test_size=min(h, max(1, len(X) // 4)))
            cv_preds, cv_actuals = [], []
            for tr_idx, te_idx in tscv.split(X):
                X_tr, X_te = X[tr_idx], X[te_idx]
                y_tr, y_te = y_clean[tr_idx], y_clean[te_idx]
                sc         = StandardScaler()
                fold_mdl   = sklearn_clone(base_model)
                fold_mdl.fit(sc.fit_transform(X_tr), y_tr)
                prd        = np.maximum(fold_mdl.predict(sc.transform(X_te)), 0.0)
            ...
            # Validation metrics (80/20 split in feature space)
            split       = max(h, len(X) // 5)
            ...
            # Final model on full feature set
            full_scaler = StandardScaler()
            X_full, y_full, _ = MLFeatureEngineering.build_features(y, period=period)
            ...
            future_y = list(y.copy())
            preds: List[float] = []
            for _ in range(h):
                X_fut, _, _ = MLFeatureEngineering.build_features(
                    np.array(future_y, dtype=float), period=period)
                if len(X_fut) == 0:
                    break
                p = float(max(0.0,
                              full_mdl.predict(full_scaler.transform(X_fut[-1:]))[0]))
                preds.append(p)
                future_y.append(p)
            ...
            # Bootstrap CI from validation residuals
            lo, hi = StatUtils.bootstrap_ci(
    ...
    def run_random_forest(self, y: np.ndarray,
                           period: int = 12) -> Optional[ForecastResult]:
    ...
    def run_gradient_boosting(self, y: np.ndarray,
                               period: int = 12) -> Optional[ForecastResult]:
    ...
    def run_ridge(self, y: np.ndarray,
                  period: int = 12) -> Optional[ForecastResult]:
        if not self.config.use_ridge:
            return None
        return self._run_ml_model(
            y, Ridge(alpha=1.0), "Ridge Regression", period,
        )

    # ── Stacked Ensemble (OOF-based, no leakage) ──────────────────────────────

    def build_ensemble(self,
                       forecasts: Dict[str, Optional[ForecastResult]],
                       y: np.ndarray,
                       period: int = 12) -> ForecastResult:
        """
        Stacking ensemble using inverse CV-sMAPE weights (non-leaky).
        Falls back to validation sMAPE if CV scores unavailable.
        """
    ...
    def forecast(self, y: np.ndarray,
                 period: int = 12) -> Tuple[Dict, ForecastResult, str]:
        runners = [
...
class SalesAnalyzer:
    ...
    def decompose(y: np.ndarray,
                  period: int = 12) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        Additive decomposition via centred moving average.
        Correct symmetric padding for both even and odd pad lengths.
        """
        n = len(y)
        if n < 2 * period:
            trend    = np.full(n, float(np.mean(y)))
            seasonal = np.zeros(n)
            return trend, seasonal, y - trend

        kernel    = np.ones(period) / float(period)
        trend_raw = np.convolve(y, kernel, mode="valid")
        n_valid   = len(trend_raw)
        pad_total = n - n_valid
        pad_l     = pad_total // 2
        pad_r     = pad_total - pad_l      # handles both even and odd

        trend = np.concatenate([
            np.full(pad_l, trend_raw[0]),
            trend_raw,
            np.full(pad_r, trend_raw[-1]),
        ])
        assert len(trend) == n, f"Trend length mismatch: {len(trend)} vs {n}"

        detrended = y - trend
        s_proto   = np.array([
            float(np.mean(detrended[i::period])) for i in range(period)
        ])
        s_proto  -= float(s_proto.mean())  # zero-mean seasonal
    ...
    def detect_anomalies(series: "pd.Series",
        ...
        rm   = series.rolling(6, center=True, min_periods=1).mean()
        rs   = series.rolling(6, center=True, min_periods=1).std().fillna(1.0)
        rs   = rs.replace(0, 1.0)   # avoid division by zero
        z    = np.abs((series - rm) / rs)
        idx  = z[z > threshold].index
        if len(idx) == 0:
            return pd.DataFrame(columns=["value", "z_score", "type"])
        anom  = series.loc[idx]
    ...
    def compute_statistics(df: "pd.DataFrame",
                            roi_ci: Optional[Dict] = None) -> Dict:
        ...
        try:
            cagr = ((last / max(first, 1.0)) ** (1.0 / n_years) - 1.0) * 100.0
        except Exception:
            cagr = 0.0
        ...
        return {
            ...
            "std_dev":                std_m,
            "cv":                     (std_m / avg_m * 100.0) if avg_m else 0.0,
            "cagr":                   cagr,
            "best_product":           _best("product"),
            "best_region":            _best("region"),
            "total_returns":          returns_total,
            "return_rate":            (returns_total / total * 100.0) if total else 0.0,
    ...
    def analyze(self, df: "pd.DataFrame") -> SalesAnalysis:
        ...
        # Auto-detect or honour configured seasonality period
...
class ChartGenerator:
    ...
    def _save(self, fig: "plt.Figure", name: str, dpi: int = 150) -> Path:
    ...
    def product_comparison(self, analysis: SalesAnalysis) -> Path:
        ...
        axes[0].set_title("Revenue by Product Over Time",
    ...
    def residual_diagnostics_chart(self, analysis: SalesAnalysis) -> Path:
        ...
        # 2. ACF — guard against n too small
        ...
        # 4. Residual histogram with normal fit
    ...
    def generate_all(self, analysis: SalesAnalysis,
                     progress_cb: Optional[Callable] = None) -> Dict[str, Path]:
...
class PDFReportGenerator:
    ...
    def _kpi_table(self, stats: Dict) -> Table:
        primary = self._rl_color(self.config.primary_color)
        roi_str = (f"{stats['marketing_roi']:.1f}x  "
                   f"[{stats['marketing_roi_ci_lo']:.1f}x – "
                   f"{stats['marketing_roi_ci_hi']:.1f}x]")
        kpis = [
            ("Total Revenue",           f"${stats['total_revenue']:,.0f}"),
            ("Avg Monthly Revenue",     f"${stats['avg_monthly_revenue']:,.0f}"),
            ("CAGR",                    f"{stats['cagr']:.1f}%"),
            ("Return Rate",             f"{stats['return_rate']:.2f}%"),
            ("Marketing ROI (95% CI)",  roi_str),
            ("Best Product",            str(stats["best_product"])),
            ("Best Region",             str(stats["best_region"])),
            ("Revenue Volatility (CV)", f"{stats['cv']:.1f}%"),
        ]
    ...
    def _add_chart(self, path: Optional[Path],
                   width: float = 6.5, caption: str = "") -> List:
        if not path or not Path(path).exists():
            return []
        elements: List = []
        try:
            img = RLImage(str(path), width=width * inch, height=width * 0.42 * inch)
            img.hAlign = "CENTER"
            elements.append(img)
            if caption:
    ...
    def generate(self, analysis: SalesAnalysis,
                 charts: Dict[str, Path],
    ...
    def _build_story(self, analysis: SalesAnalysis, charts: Dict) -> List:
        ...
        story.append(Paragraph("Executive KPI Summary", S["SecHead"]))
        story.append(self._kpi_table(st))
        story.append(Spacer(1, 0.3 * inch))
        story.append(PageBreak())

        # ── §1 Historical ─────────────────────────────────────────────────────
        story.append(Paragraph("1. Historical Sales Performance", S["SecHead"]))
        story.append(Paragraph(
        ...
        # ── §3 Products ───────────────────────────────────────────────────────
        story.append(Paragraph("3. Product-Level Analysis", S["SecHead"]))
        ...
        if not analysis.anomalies.empty:
            ...
            story.append(Paragraph("Detected Anomalies", S["SubHead"]))
            story.append(Paragraph(
        ...
        # ── §4 Forecast ───────────────────────────────────────────────────────
        story.append(Paragraph("4. Demand Forecasting Results", S["SecHead"]))
        story.append(Paragraph(
        ...
        story.append(Spacer(1, 0.15 * inch))
        story.append(Paragraph(
            "Model Performance Comparison (sMAPE / MASE / RMSE / R²)", S["SubHead"]))
        story += self._add_chart(charts.get("model_comparison"),
                                  caption="Figure 7: Model Comparison")
        story.append(Spacer(1, 0.15 * inch))
        story.append(self._metrics_table(analysis))
        story.append(Spacer(1, 0.2 * inch))

        story.append(Paragraph("Residual Diagnostics", S["SubHead"]))
        ...
        # ── §5 Monthly detail ─────────────────────────────────────────────────
        story.append(Paragraph("5. Monthly Forecast Detail", S["SecHead"]))
        story.append(Paragraph(
        ...
        story.append(Paragraph(
            "<i>Auto-generated by Sales Forecasting System v5.0. "
            ...
            "ML models: PACF-guided features, iterative multi-step forecast, bootstrap CI. "
            "Ensemble: inverse CV-sMAPE stacking (walk-forward, non-leaky). "
            "sMAPE symmetric and bounded [0,200]; MASE scaled to naïve baseline. "
            "Marketing ROI includes 95% bootstrap CI. "
            "Charts dir: system tempdir (cross-platform). "
...
class ExcelReportGenerator:
    ...
    def _data_cell(self, ws: Any, row: int, col: int,
    ...
    def _write_data_row(self, ws: Any, row: int, values: List[Any],
    ...
    def _write_title_block(self, ws: Any, title: str, subtitle: str = "") -> int:
        """Write title row(s) and return the next empty row number."""
    ...
    def _sheet_summary(self, wb: Any, analysis: SalesAnalysis) -> None:
        ...
        kpis = [
            ...
            ("Peak Month",                st["peak_month"].strftime("%b %Y")),
            ("Peak Revenue",              f"${st['peak_value']:,.2f}"),
            ("Trough Month",              st["trough_month"].strftime("%b %Y")),
    ...
    def _sheet_forecast(self, wb: Any, analysis: SalesAnalysis) -> None:
        ...
        for i, (dt, fc, lo, hi) in enumerate(
            ...
            mom_pct  = (fc_f - prev) / max(abs(prev), 1e-10) * 100.0  # as percentage number
            cum     += fc_f
            shade    = (i % 2 == 0)
            vals     = [dt.strftime("%b %Y"),
                        round(fc_f, 2), round(lo_f, 2), round(hi_f, 2),
                        round(hi_f - lo_f, 2),
                        round(mom_pct, 2),   # store as number, format below
                        round(cum, 2)]
            self._write_data_row(ws, r, vals, shade=shade)
            # MoM: value is already a percentage number (e.g. 3.5 = 3.5%)
            mom_cell = ws.cell(row=r, column=6)
            mom_cell.number_format = '+0.00;-0.00;0.00'   # show sign, no % symbol
    ...
    def _sheet_history(self, wb: Any, analysis: SalesAnalysis) -> None:
        ...
        for i, row in enumerate(monthly.itertuples(index=False)):
            ...
            vals  = list(row)
            self._write_data_row(ws, r, vals, shade=shade)
            # Colour YoY and MoM columns (indices 4 and 5, 1-based = cols 5 and 6)
    ...
    def generate(self, analysis: SalesAnalysis,
        ...
        try:
            ...
            # Embed chart images in individual sheets
...
class ScrollFrame(tk.Frame):
    ...
    def _on_mousewheel(self, e: Any) -> None:
...
class SalesForecastApp:
    ...
    def __init__(self, root: tk.Tk):
        ...
        # Thread-safe state (written only from worker, read only from main via queue)
        self._chosen_output: Optional[Path] = None
        self._cancel_token: Optional[CancelToken] = None
        self._q: queue.Queue = queue.Queue()
        self._running = False
        # analysis is posted to main thread via queue — never read directly from worker
    ...
    def _build_header(self) -> None:
        ...
        tk.Label(info, text="Sales Forecasting System",
    ...
    def _build_models_card(self, parent: tk.Frame) -> None:
        ...
        models = [
            (self.v_use_arima, "ARIMA  (auto order, exact MLE)",
             "Grid-search by AIC; CI via ψ-weight variance propagation; pandas 2.x safe"),
            ...
            (self.v_use_rf,    "Random Forest  (200 trees, PACF lags)",
             "Bootstrap CI; PACF-guided lag selection; sklearn.base.clone for CV safety"),
            (self.v_use_gb,    "Gradient Boosting  (200 rounds)",
             "Bootstrap CI; walk-forward cross-validation score reported"),
    ...
    def _pipeline(self, cfg: ForecastConfig,
        ...
        try:
            ...
            try:
                shutil.rmtree(cfg.charts_dir, ignore_errors=True)
            except Exception:
                pass

            report("Complete! ✓", 100)
            # Post analysis to main thread via queue (thread-safe)
    ...
    def _on_done(self, result: Dict) -> None:
        ...
        files    = result["files"]
        cfg      = result["cfg"]
        # Thread-safe: we are now on the main thread (called from _poll)
    ...
    def _populate_results(self, analysis: SalesAnalysis,
                           files: Dict, cfg: ForecastConfig) -> None:
        st  = analysis.statistics
        ens = analysis.ensemble_forecast
...
def main() -> None:
    if _IMPORT_ERROR:
        root = tk.Tk()
        root.after(0, lambda: root.state('zoomed'))   # optional (maximize)
    ...
    app = SalesForecastApp(root)
    app._log_msg("Sales Forecasting System v5.0 ready.", "ok")
    app._log_msg("⚙  Setup tab → configure → Run.", "accent")
    app._log_msg("v5.0 fixes: ARIMA undiff (d>1) · HW seasonal init · PACF lags ·", "muted")
    app._log_msg("           bootstrap CI · pandas 2.x dates · xls engine · BOM ·", "muted")
    app._log_msg("           decomp pad · ACF guard · Excel MoM format · thread safety", "muted")
    app._log_msg("Supported: .csv  .tsv  .xlsx  .xlsm  .xls  .json  .parquet", "muted")
    app._log_msg("Required columns: 'date', 'sales'", "muted")
