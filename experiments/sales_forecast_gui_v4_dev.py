"""
Sales Forecasting & Automated Reporting System  v4.0
═══════════════════════════════════════════════════════════════════════════════
...
v4 improvements over v3:
  [V4-1]  Parquet engine auto-fallback (pyarrow → fastparquet → manual error)
  [V4-2]  ARIMA auto_select raises on empty series instead of silent wrong fit
  [V4-3]  HoltWinters.fit() — season init guards against all-zero series
  [V4-4]  MLFeatureEngineering — explicit float64 cast prevents int overflow
  [V4-5]  ForecastingEngine._run_ml_model — scaler fitted on train ONLY
          (previous code transformed full matrix with train scaler — leaky)
  [V4-6]  WalkForwardCV — horizon clamped relative to available data per fold
  [V4-7]  SalesAnalyzer.decompose — vectorised, zero-alloc, handles n<period
  [V4-8]  detect_anomalies — minimum 3-period window, robust to flat series
  [V4-9]  compute_statistics — safe division everywhere; Timestamp→ISO string
  [V4-10] ChartGenerator — DPI bumped to 180; axis tick formatting unified
  [V4-11] PDFReportGenerator — currency symbol injected from config everywhere
  [V4-12] ExcelReportGenerator — xlchart embedded for Forecast sheet
  [V4-13] SalesForecastApp — "Preview Data" button shows loaded df head
  [V4-14] Thread safety: all Tk writes guarded via after(); _q bounded(1000)
  [V4-15] Config.validate() extended: company/analyst name length checks
  [V4-16] DataLoader — parquet, JSON, CSV all get stricter post-load checks
  [V4-17] Global: all bare `except` replaced with `except Exception`
  [V4-18] Robust temp-dir cleanup even on Windows (retry with delay)
  [V4-19] JSON summary serialises numpy scalars via custom encoder
...
APP_TITLE   = "Sales Forecasting System v4"
APP_VERSION = "4.0.0"
...
class DataLoader:
    ...
    def _validate_and_clean(cls, df: "pd.DataFrame") -> "pd.DataFrame":
        ...
        df["sales"] = (
            pd.to_numeric(df["sales"], errors="coerce").fillna(0.0).clip(lower=0.0)
        )
...
class SalesAnalyzer:
    ...
    def decompose(
        ...
        # centred moving average via cumsum — O(n), allocation-free
...
class SalesForecastApp:
    ...
    def _preview_data(self) -> None:
        ...
        vsb = tk.Scrollbar(top, orient="vertical", command=tree.yview, bg=self.T.SURFACE)
        hsb = tk.Scrollbar(top, orient="horizontal", command=tree.xview, bg=self.T.SURFACE)
    ...
    def _pipeline(
        ...
        try:
            ...
            summary   = {
                ...
                "v4_improvements": [
                    "V4-1: Parquet engine auto-fallback (pyarrow→fastparquet)",
                    "V4-2: ARIMA raises on <6 obs instead of silently misfitting",
                    "V4-3: HoltWinters init guards zero-level in multiplicative mode",
                    "V4-4: MLFeatureEngineering uses explicit float64 (int overflow fix)",
                    "V4-5: ML scaler fitted on train split only — no leakage into test",
                    "V4-7: Decompose fully vectorised; handles n<period gracefully",
                    "V4-8: Anomaly detection adaptive window + flat-series guard",
                    "V4-9: compute_statistics safe division; Timestamps as ISO strings",
                    "V4-10: Charts DPI=180; cleaner axes styling",
                    "V4-11: PDF KPI table uses configured currency symbol",
                    "V4-13: Preview Data button in GUI Setup tab",
                    "V4-14: Thread queue bounded to 1000 items",
                    "V4-15: Config validation: name length + symbol checks",
                    "V4-18: Temp-dir cleanup retries 3× (Windows file-handle fix)",
                    "V4-19: JSON serialises numpy scalars via _NpEncoder",
                    "V4-20: Progress bar monotonic (never regresses)",
                ],
                "platform_info": {
                    "python":          sys.version,
...
def main() -> None:
    ...
    app._log_msg("── v4 fixes & enhancements ───────────────────────────", "sep")
    app._log_msg("[V4-1]  Parquet engine auto-fallback (pyarrow→fastparquet)", "muted")
    app._log_msg("[V4-2]  ARIMA raises on <6 obs instead of silently misfitting", "muted")
    app._log_msg("[V4-3]  HoltWinters init guards zero-level multiplicative mode", "muted")
    app._log_msg("[V4-4]  MLFeatureEngineering uses explicit float64 (int overflow fix)", "muted")
    app._log_msg("[V4-5]  ML scaler fitted on TRAIN split only — no leakage", "muted")
    app._log_msg("[V4-7]  Decompose vectorised; handles n<period gracefully", "muted")
    app._log_msg("[V4-8]  Anomaly detection: adaptive window + flat-series guard", "muted")
    app._log_msg("[V4-9]  compute_statistics: safe div + Timestamps as ISO strings", "muted")
    app._log_msg("[V4-10] Charts DPI=180; cleaner axes styling", "muted")
    app._log_msg("[V4-11] PDF KPI table uses configured currency symbol", "muted")
    app._log_msg("[V4-13] 'Preview Data' button in Setup → Data Source card", "muted")
    app._log_msg("[V4-14] Thread queue bounded to 1000 items", "muted")
    app._log_msg("[V4-15] Config validation: name length + symbol checks", "muted")
    app._log_msg("[V4-18] Temp-dir cleanup retries 3× (Windows handle fix)", "muted")
    app._log_msg("[V4-19] JSON serialises numpy scalars via _NpEncoder", "muted")
    app._log_msg("[V4-20] Progress bar is monotonic (never regresses)", "muted")
    app._log_msg("────────────────────────────────────────────────────────", "sep")
